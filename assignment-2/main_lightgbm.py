...
def load_data():
    ...
    train[target_column] = train[target_column].map(
        lambda x: build_target_column(x))
    train.head()
    ...
    n = 17
    saved_features = [list(ax.get_yticklabels())[i].get_text()
                      for i in range(n, len(list(ax.get_yticklabels())))]

    overlapped_features = ["prop_location_score2",
                           "price_usd", "position", 'srch_id', 'prop_id']
    X = X[saved_features]
    ...
    # Final output
    catergorical_col = ["prop_id", "srch_id", "visitor_location_country_id",
                        "prop_country_id", "srch_booking_window", "srch_destination_id"]
    X = X  # Training set
    y = y  # Target series
    ...
    test = pd.DataFrame(MinMaxScaler().fit_transform(test),columns=X.columns)
...
def save(X, y, groups, test, test_srch_id, test_prop_id):
    ...
    groups.to_pickle(os.path.join(
        base_dir, 'perfect_fgroups.pkl'), compression='gzip')
    test.to_pickle(os.path.join(base_dir, 'perfect_ftest.pkl'),
                   compression='gzip')
    test_srch_id.to_pickle(os.path.join(
        base_dir, 'perfect_ftest_si.pkl'), compression='gzip')
    test_prop_id.to_pickle(os.path.join(
        base_dir, 'perfect_ftest_pi.pkl'), compression='gzip')
...
def run(X, y, groups, test, test_srch_id, test_prop_id):
    ...
    for param in params_list:
        ...
        for train_inds, val_inds in gkf.split(X, y, groups):
            ...
            gc.collect()
            ranker = lightgbm.LGBMRanker(**param)
            ranker.fit(X_train, y_train, group=group_train, eval_set=[
                (X_train, y_train), (X_val, y_val)], eval_group=[group_train, group_val], eval_at=5, verbose=20, early_stopping_rounds=200, eval_metric='ndcg')
            val_metrics_li.append(ranker.evals_result_['valid_1']['ndcg@5'][-1])
    ...
    ranker = lightgbm.LGBMRanker(**best_param)
    ranker.fit(X, y, group=group, verbose=20, eval_set=[
        (X, y)], eval_group=[group], eval_metric='ndcg', early_stopping_rounds=200)
...
def predict(model, X, srch_id, prop_id):
    ...
    test_data_srch_id_prop_id = pd.concat(
        [srch_id, prop_id], axis=1, sort=False)
    ...
    print("Saving predictions into submission-lightgbm.csv")
    test_data_srch_id_prop_id[["srch_id", "prop_id"]].to_csv(
        os.path.join(base_dir, "submission-lightgbm.csv"), index=False
    )
...
def train_and_predict(X, y, groups, test, test_srch_id, test_prop_id):
    params = {'objective': "lambdarank",
              ...
              'boosting': 'dart'}
    group = groups2group(groups)
    categorical_features_numbers = get_categorical_column(X)
    y = y.clip(0,2)
    print(np.unique(y))
    ranker = lightgbm.LGBMRanker(**params)
    ranker.fit(X, y, group=group, verbose=20, eval_set=[
        (X, y)], eval_group=[group], eval_metric='ndcg', eval_at=5, early_stopping_rounds=100, categorical_feature=categorical_features_numbers)
    import joblib
    joblib.dump(ranker, 'lgb.pkl')
    ranker = joblib.load('lgb.pkl')
    # ranker.save_model('lgbm_model.mdl')
    categorical_features_numbers = get_categorical_column(test)
    predictions = ranker.predict(test, categorical_feature=categorical_features_numbers)
    test_data_srch_id_prop_id = pd.concat(
        [test_srch_id, test_prop_id], axis=1, sort=False)
    test_data_srch_id_prop_id.columns
    test_data_srch_id_prop_id["prediction"] = predictions
    test_data_srch_id_prop_id = test_data_srch_id_prop_id.sort_values(
...
if __name__ == "__main__":
    ...
    if not LOAD:
        ...
        X = pd.read_pickle(os.path.join(
            base_dir, 'perfect_fX.pkl'), compression='gzip')
        y = pd.read_pickle(os.path.join(
            base_dir, 'perfect_fyy.pkl'), compression='gzip')
        groups = pd.read_pickle(os.path.join(
            base_dir, 'perfect_fgroups.pkl'), compression='gzip')
        test = pd.read_pickle(os.path.join(
            base_dir, 'perfect_ftest.pkl'), compression='gzip')
        test_srch_id = pd.read_pickle(os.path.join(
            base_dir, 'perfect_ftest_si.pkl'), compression='gzip')
        test_prop_id = pd.read_pickle(os.path.join(
            base_dir, 'perfect_ftest_pi.pkl'), compression='gzip')
        print(np.unique(y))
